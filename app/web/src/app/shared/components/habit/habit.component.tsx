...
import {
  AchievementData,
  GlobalHabitData,
} from "../../../../entitites/user/models/user.slice";
...
function Habit({
  ...
  const habitMasteries = achievements
    .filter((a) => {
      if (isGlobal) {
    ...
    <Card sx={{ boxShadow: 2, padding: 2 }}>
      <CardContent>
        <Box display="flex" alignItems="center" gap="1rem" marginBottom="1rem">
          ...
          {isGlobal && (
            <Chip label="Global" color="primary" size="small" sx={{ ml: 1 }} />
          )}
          {isCompletedToday() && (
            <Chip
          ...
          {habitMasteries.map((m, i) => (
            <img
              width={40}
              height={40}
              src={masteryImages[m.Mastery?.id - 1]}
              key={i}
            />
          ))}
        </Box>
        ...
        <Typography variant="body1" sx={{ mb: 1 }}>
          Days habit should be done:{" "}
          <Typography component="span" fontWeight="600" fontSize="1.2rem">
            {habit?.everyday ? "Daily" : formatDays(habit?.days || [])}
          </Typography>
        </Typography>
        <Typography variant="body1" sx={{ mb: 1 }}>
          ...
          <Typography component="span" fontWeight="600" fontSize="1.2rem">
            {habit?.lastCompletion
              ? formatDate(habit?.lastCompletion)
              : "Not completed yet"}
          </Typography>
        </Typography>
        <Stack direction="row" spacing={1}>
          ...
          {!isGlobal && deleteHandler && (
            <Button
              ...
              color="error"
              size="small"
              onClick={() => deleteHandler(habit?.id || 0)}
            >
              Delete
            </Button>
          )}
          <Button
            ...
            onClick={() => completeHandler(habit?.id || 0)}
          >
